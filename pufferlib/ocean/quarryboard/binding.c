...
#include "quarryboard.c"

#define Env Quarryboard
#define c_step q_step
#define c_reset q_reset
#define c_close q_close
#define c_render q_render
...
static int my_init(Env* env, PyObject* args, PyObject* kwargs) {
    ...
    PyObject* start_crystals_obj = PyDict_GetItemString(kwargs, "start_crystals");
    env->start_crystals = (start_crystals_obj && PyLong_Check(start_crystals_obj)) ? (int)PyLong_AsLong(start_crystals_obj) : 5;
    
    PyObject* start_stones_obj = PyDict_GetItemString(kwargs, "start_stones");
    env->start_stones = (start_stones_obj && PyLong_Check(start_stones_obj)) ? (int)PyLong_AsLong(start_stones_obj) : 5;
...
static int my_log(PyObject* dict, Log* log) {
    ...
    assign_to_dict(dict, "p1_stone_collected", log->p1_stone_collected);
    assign_to_dict(dict, "p2_stone_collected", log->p2_stone_collected);
    assign_to_dict(dict, "p3_stone_collected", log->p3_stone_collected);
    assign_to_dict(dict, "p4_stone_collected", log->p4_stone_collected);
    ...
    assign_to_dict(dict, "p3_return", log->p3_return);
